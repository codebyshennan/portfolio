---
title: Bearing
description: AI-powered investment memo system for Iterative that runs parallel research agents to verify founder claims, map the SEA competitive landscape, and produce a structured Decision Brief before drafting the full IC memo.
date: 2025-03-01
...
Writing investment memos is slow. Each one requires founder background research, competitive landscape analysis, market sizing, and synthesis into a structured document. That is easily a full day's work per deal. With hundreds of inbound deals, the team couldn't write memos for every promising one. I built Bearing to run the research pipeline in parallel and produce both a Decision Brief and a full IC memo automatically, so the team could spend time refining analysis instead of gathering it.
...
Claude Sonnet 4.6 with extended thinking (8,000-token budget) reads the full research context and evaluation scores, then produces the Decision Brief: an Invest / Pass / Watch recommendation with a structured rationale covering the bull case, bear case, and the single biggest risk. The thinking budget is intentional because the brief needs to work through contradictions in the research (founder claims vs. competitive data, evidence tier vs. founder confidence) before committing to a recommendation.
...
- EnrichLayer over Proxycurl: EnrichLayer's API returns richer career data at lower cost for the volume Bearing processes. The Neon DB cache further reduces spend because most repeat founders show up in multiple deals, so the 90-day TTL pays off quickly.
